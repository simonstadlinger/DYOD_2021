...
std::shared_ptr<const Table> TableScan::_on_execute() {
  ...
  // create empty position list for reference segments
  ...
  if (probe_ref_segment) {
    ...
    // set input table to referenced table
    ...
    for (auto position : *(probe_ref_segment->pos_list())) {
      ...
      // get segment for scan operation
      ...
      resolve_data_type(col_type, [&](const auto data_type_t) {
        ...
        // assert that type of scan value and type of segment (column) are the same
        Assert((_search_value.type() == typeid(Type)), "Type of search value and column should be the same");

        // cast to value segment to check whether we have value segment or dictionary segment
        // if cast returns null ptr we have dictionary segment
        // if successful cast we have value segment
        auto value_segment = std::dynamic_pointer_cast<ValueSegment<Type>>(segment);

        // get corresponding comparator method for ScanType
        auto _comparator = _get_comparator_method<Type>();

        if (value_segment) {
          // value Segment

          // get actual value for position list entry
    ...
    for (auto chunk_id = ChunkID{0}; chunk_id < chunk_count; ++chunk_id) {
      ...
      resolve_data_type(col_type, [&](const auto data_type_t) {
        ...
        // cast to value segment to check whether segment is value segment or dictionary segment
        auto value_segment = std::dynamic_pointer_cast<ValueSegment<Type>>(segment);

        // get corresponding comparator method for ScanType
        auto _comparator = _get_comparator_method<Type>();

        // assert that column type and search value type are the same
        Assert((_search_value.type() == typeid(Type)), "Type of search value and column should be the same");
        ...
        if (value_segment) {
          ...
          for (auto chunk_offset = ChunkOffset{0}; chunk_offset < chunk_size; ++chunk_offset) {
            ...
            // compare and append to position list if true
            if (_comparator(type_cast<Type>(_search_value), comparison_value)) {
              pos_list->push_back(RowID{chunk_id, chunk_offset});
            }
          }
  ...
  // create one single chunk for reference segments
  std::shared_ptr<Chunk> final_chunk = std::make_shared<Chunk>(column_count);

  // for each col, create reference segment and add to chunk
